...
class Table:
    ...
    def add_used_column(self, column): #记录某个表已用过的列
...
class Parser:
    def __init__(self, db_info):
        ...
        #  等号和in均解析为=，like被解析为'~~'
        self.support_op = {'=': 0, '>': 1, '<': 2, '>=': 3, '<=': 4, 'BETWEEN': 5, '~~': 6}
    ...
    def gain_candidates(self):
        for tb in self.table_info:
            ...
            if tb_info.table_name != tb: # 判断是否取了别名
            ...
            if len(tb_info.join_conditions) > 0:
                for cond in tb_info.join_conditions:
                    ...
                    if _other_table != tb and not (_other_table+"#"+cond.column in joined_id):
                        ...
                        joined_index.add(tb+"#"+cond.column)  # 单列join
                        if _other_table in joined_table:  # 只有当join的两个表出现两次以上按列join时，
                            ...
                            c1 = set()
                            c2 = set()
                            for _n_attr in range(2, len(_p_j_col1)+1):  # 左闭右开的范围，假设两个表在3个列上join，那么对这3个列从[2,3+1)上取全排列组合
                                permutations = list(itertools.permutations(_p_j_col1, _n_attr))  # 返回指定大小_n_attr的全排列
            ...
            if len(tb_info.conditions) > 0:
                ...
                # 遍历conditions，where中的等值/范围/like/between/in，添加到候选索引集中
                ...
                # permutation join中的列拆开后，与等值连接中的列做全排，大小为[2,len(_n_cols) + 1]，然后与mix_indexes合并去重
                ...
                self.index_candidates = self.index_candidates | temp
            # handle group by
            if len(tb_info.group.keys()) != 0:
                group_col = tb_info.table_name+'#'
                for col in tb_info.group.keys():
                    group_col = group_col + col + ','
                group_col = group_col.rstrip(',')
                self.index_candidates.add(group_col)
            # handle order by
            if len(tb_info.order.keys()) != 0:
                order_col = tb_info.table_name+'#'
                for col in tb_info.order.keys():
                    order_col = order_col + col + ','
                order_col = order_col.rstrip(',')
                self.index_candidates.add(order_col)
            '''new_added = set()
    ...
    def parse_select(self, select_stmt):
        ...
        if 'whereClause' in select_stmt.keys():
            ...
            self.parse_where_clause(where_clause) #追加一个condition条件
        ...
        # (4)parse order by,和group by处理逻辑一致
    ...
    def parse_range_var(self, range_var):
        ...
        if alias_name in self.table_info.keys(): #能够避免取了别名，或者相同的table出现时重复建表
            # subquery/sublink
            return
        # table_info是一个字典，key是表名alias_name，value是一个对应的Table对象
        # 设置某个表（self.table_info[alias_name]）与该表对象包含的所有列（self.db_info[table_name]，这是一个字典）
    ...
    def parse_a_expr(self, a_expr, is_in_or, is_target): #当target为true时没有处理
        # currently, we only consider =/>/<
        op = a_expr['name'][0]['String']['str'] #获取操作符
        # deal with left
        ...
        if op == 'BETWEEN' or op == '~~':  # 处理between或like时，归到where条件中去
    ...
    def parse_targets(self, target_list): # 解析返回值
